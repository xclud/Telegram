...
using TLSharp.Requests;
using TLSharp.Utils;

namespace TLSharp.Network
{
    internal class MtProtoSender
    {
        ...
        {
            return confirmed ? _client._session.Sequence++ * 2 + 1 : _client._session.Sequence * 2;
        }
        ...
        public async Task Send(byte[] packet, TLSchema.TLMethod request)
        {
            request.MessageId = _client._session.GetNewMessageId();
            ...
            using (MemoryStream plaintextPacket = makeMemory(8 + 8 + 8 + 4 + 4 + packet.Length))
            {
                ...
                {
                    plaintextWriter.Write(_client._session.Salt);
                    plaintextWriter.Write(_client._session.Id);
                    ...
                    ciphertext = AES.EncryptAES(Helpers.CalcKey(_client._session.AuthKey.Data, msgKey, true), plaintextPacket.GetBuffer());
                }
            }
            ...
            using (MemoryStream ciphertextPacket = makeMemory(8 + 16 + ciphertext.Length))
            {
                ...
                {
                    writer.Write(_client._session.AuthKey.Id);
                    writer.Write(msgKey);
        ...
        private Tuple<byte[], ulong, int> DecodeMessage(byte[] body)
        {
            ...
            using (var inputReader = new BinaryReader(inputStream))
            {
                ...
                AESKeyData keyData = Helpers.CalcKey(_client._session.AuthKey.Data, msgKey, false);
        ...
        public async Task<byte[]> Receive(TLSchema.TLMethod request)
        {
        ...
        private bool processMessage(ulong messageId, int sequence, BinaryReader messageReader, TLSchema.TLMethod request)
        {
            // TODO: check salt
        ...
        private bool HandleGzipPacked(ulong messageId, int sequence, BinaryReader messageReader, TLSchema.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();

            var decompressedFileStream = new MemoryStream();
            using (GZipStream decompressionStream = new GZipStream(new MemoryStream(Serializers.Bytes.read(messageReader)), CompressionMode.Decompress))
            {
                decompressionStream.CopyTo(decompressedFileStream);
            }
            using (var packedStream = new MemoryStream(decompressedFileStream.ToArray(), false))
        ...
        private bool HandleRpcResult(ulong messageId, int sequence, BinaryReader messageReader, TLSchema.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
        ...
        private bool HandleBadServerSalt(ulong messageId, int sequence, BinaryReader messageReader, TLSchema.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
        ...
        private bool HandlePong(ulong messageId, int sequence, BinaryReader messageReader, TLSchema.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
        ...
        private bool HandleContainer(ulong messageId, int sequence, BinaryReader messageReader, TLSchema.TLMethod request)
        {
            uint code = messageReader.ReadUInt32();
